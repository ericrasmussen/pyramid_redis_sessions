...
class TestRedisSessionFactory(unittest.TestCase):
    ...
    def test_configuration_from_url(self):
        url_without_db = 'redis://username:password@localhost:6379'
        redis = DummyRedis.from_url(url_without_db)
        request = testing.DummyRequest()
        request.registry._redis_sessions = redis
        inst = self._makeOne(request)
    ...
    def test_configuration_from_url_db_specified(self):
        url_with_db = 'redis://username:password@localhost:6379/1'
        redis = DummyRedis.from_url(url_with_db)
        request = testing.DummyRequest()
        request.registry._redis_sessions = redis
        inst = self._makeOne(request)
        self.assertEqual(inst.redis.host, 'localhost')
